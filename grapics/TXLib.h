//=================================================================================================================
//           [These sections are for folding control  in Code::Blocks]         [$Date: 2021-01-05 12:10:19 +0400 $]
...
#define _TX_VER      _TX_v_FROM_CVS ($VersionInfo: , TXLib.h, 00173a, 168, 2021-01-05 12:10:19 +0400, "Ded (Ilya Dedinsky, http://txlib.ru) <mail@txlib.ru>", $)
#define _TX_VERSION  _TX_V_FROM_CVS ($VersionInfo: , TXLib.h, 00173a, 168, 2021-01-05 12:10:19 +0400, "Ded (Ilya Dedinsky, http://txlib.ru) <mail@txlib.ru>", $)
#define _TX_AUTHOR   _TX_A_FROM_CVS ($VersionInfo: , TXLib.h, 00173a, 168, 2021-01-05 12:10:19 +0400, "Ded (Ilya Dedinsky, http://txlib.ru) <mail@txlib.ru>", $)
...
inline void txDrawMan (int x, int y, int sizeX, int sizeY, COLORREF color, double handL, double handR, double twist,   //
    ...
    txNotifyIcon (4, (const char*)!! (L+'L')[msg], "\n%s\n", msg + ((unsigned) (((count -=- 1) ^=! 1) ^=~ ((0)^(0)) +1) % 3)["\"<"]);  //-V112 //-V542
...
//! @param   pixels <i>When bitmap == NULL, a DIBSection will be created (see above) and this parameter will be
//!                    associated with its pixels colors array pointer. See txCreateDIBSection() function below.
//!                    This parameter may be omitted.</i>
//!
...
//!
//! @note    Стандартная функция @c AlphaBlend из Win32 API @b может масштабировать изображение. В txAlphaBlend это
//!          @b убрано для упрощения использования. If you still need image scaling, use original function AlphaBlend
//!          and don't mess with stupid TX-based tools. (See implementation of txAlphaBlend in TX Library source:
//!          open @ref TXLib.h file in your editor and search for txAlphaBlend function @b definition.)
//!
...
//!                  {
//!                  int rot = (short) HIWORD (wParam) / WHEEL_DELTA;
//!
//!                  RECT rect = {};
...
//!
//!                  wheel = true;
//!                  MoveWindow (window, rect.left - rot, rect.top - rot, size.x + rot*2, size.y + rot*2, true);
//!                  }
//!
//!              static int i = 0;
//!              if (i++ % 15 == 0)
...
$   if (_txMain && _txConsole)
        {$ _txConsole_Detach (waitableParent && !externTerm); }  //-V560

$   if (waitableParent && paused && _txNOP (_TX_ALLOW_KILL_PARENT))
        {
$       bool parentKilled = _txKillProcess (parent);
...
HDC txCreateDIBSection (double sizeX, double sizeY, RGBQUAD** pixels /*= NULL*/)
    {
$1  HDC dc = txCreateCompatibleDC (sizeX, sizeY, NULL, pixels);
...
$   bool ok = true;
    ...
    //
    // * Вызов стандартной функции Win32::TransparentBlt() *
    //
    // Ниже - это вызов Win32::TransparentBlt(). Он так устроен, что, если вдруг что-то не получилось (т.е. Win32::TransparentBlt()
    // вернет 0), то тогда будет вызвана Win32::BitBlt(), которая просто скопирует картинку без учета прозрачного цвета. Погуглите
    // "Windows TransparentBlt function" и почитайте про ее параметры.
    //
...
$   if (Win32::TransparentBlt) // Только то, что эти параметры передаются одинаковыми, не дает возможность менять масштаб картинки!        // <<--
        {                      //                           //                                       vvvvv          vvvvvv                 // <<--
...
$   RGBQUAD* buf = new (std::nothrow) RGBQUAD [size.x * size.y];            //-V121
$   ok &= (buf != NULL);
...
$   _txPrintV (stream, format, n,   fmt);
    ...
    if (fmt[0] == '%' && fmt[1] == '*' && fmt[2] == '.' && fmt[3] == '*') {$ stream << std::setw (width) << std::setprecision (prec + 1); }  //-V2006
...
inline std::ostream& operator << (std::ostream& stream, const POINT& point)
    ...
    if (_TX_ARGUMENT_FAILED (&point))  return stream;  //-V778

   if (&point) stream << "{ x: " << point.x << ", y: " << point.y << " }";            // NOLINT (clang-diagnostic-undefined-bool-conversion)
    else        stream << "(null)";
...
inline std::ostream& operator << (std::ostream& stream, const SIZE& size)
    {
    if (_TX_ARGUMENT_FAILED (&stream)) return stream;
    if (_TX_ARGUMENT_FAILED (&size))   return stream;  //-V778
...
inline std::ostream& operator << (std::ostream& stream, const RECT& rect)
    {
    if (_TX_ARGUMENT_FAILED (&stream)) return stream;
    if (_TX_ARGUMENT_FAILED (&rect))   return stream;  //-V778
